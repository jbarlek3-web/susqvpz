...
try {
  ...
  try {
    ...
    ({ createServer: createViteServer } = await import(pathToFileURL(resolved).href));
  } catch {
  ...
  try {
    if (createViteServer) {
      ...
      try {
        ...
        const devUrl = server.resolvedUrls?.local?.[0] ?? `http://127.0.0.1:${port}`;
        return await body(devUrl);
      } finally {
        await server.close();
      }
    } else {
      const { createServer: createHttpServer } = await import("node:http");
      const plugin = appEnvPlugin();
      let middleware = null;
      plugin.configureServer?.({
        config: { env: { VITE_AUTH_ENABLED: process.env.VITE_AUTH_ENABLED } },
        middlewares: {
          use: (fn) => {
            middleware = fn;
          },
