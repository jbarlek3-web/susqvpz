...
try {
  ...
  try {
    ...
    const resolvedPath = require.resolve("vite");
    ({ loadEnv } = await import(pathToFileURL(resolvedPath).href));
  } catch {
    loadEnv = (_mode, _root, prefixes = "VITE_") => {
      const prefixList = Array.isArray(prefixes) ? prefixes : [prefixes];
      const env = {};
      for (const [key, value] of Object.entries(process.env)) {
        if (prefixList.some((p) => p === "" || key.startsWith(p)) && typeof value === "string") {
          env[key] = value;
        }
      }
      return env;
...
test("this account-enabled app ships auth on", () => {
...
test("vite loadEnv resolves the wrapped value", () => {
  ...
  try {
    ...
    delete baseEnv.VITE_AUTH_ENABLED;
    const merged = mergeAppEnv(readAppEnv(root), baseEnv);
